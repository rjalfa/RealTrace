...
{
	register int localId = threadIdx.x * TY + threadIdx.y;
	...
	__shared__ Ray ray[TX*TY];
	
	ray[localId] = rays[index];
	...
	intersect(triangles, num_triangles, ray + localId, &ug, in_coeff);
	if (in_coeff < EPSILON || ray[localId].direction == make_float3(0, 0, 0)) return;
	//bool reflect_over_refract = false;
	//Do one time intersection
	float3 finalColor = AMBIENT_COLOR;
	register Triangle* intersected = 0;
	if (ray[localId].has_intersected) {
		intersected = ray[localId].intersected;
		float3 I = normalize(ray[localId].direction);
		...
		finalColor = get_light_color(get_point(ray + localId, ray[localId].t), N, &l, intersected, I);
		...
		float3 bias = N * 1e-4f;
		float3 hitPoint = ray[localId].getPosition();
